...
//Structure for most of the graphic elements
...
//For detecting the cursor position and setting the speed according to the distance 
...
//Creates the platforms the balls land on
...
//To define the coordinates of the platforms
...
//Checks collision between objects and platforms using aabb collision
                ...
                if (glm::length(difference) <= C->radius) {
                        switch(distance(dot_prod, max_element(dot_prod, dot_prod+4)))
                        {
                                ...
                                case DOWN:
                                        ...
                                        if(abs(C->vy) < 0.05)
                                                C->vy = 0;
                                        //If the speed is less than a certain value
                                        //then ball moves with the paltform
...
//Highlights the path of the ball as it is fired from cannon
void createPath()
{
    object dot;
    dot.x=ball.x;
...
//the goal the balls are to be thrown into
void createGoal()
{
        static const GLfloat vertex_buffer_data [] = {
                3.2,-2,0, // vertex 1
...
//Checks if the objects is in the goal area;
...
//Creating the grass on the floor
void createGrass()
{
        object grass;
        float i;
...
//Defines the location of the blocks
void defineBlocks()
{
...
//Creatign the ball fired from cannon
...
//Create the power bar for firing the ball;
...
//Creates the cannon to fire the ball
void createCannon()
{
...
//Function to reduce the speed of moving objects 
...
// calls function to check Collision between two balls
...
//Updates the objects position and velocities and collisions in this function
...
//Function for creating rectangles for 7segment display
...
void draw ()
{
        ...
        /*Implements the zooming and panning function
         * zoom stores the ammount of zooming and pan 
         * is the ammount of panning in the projection matrix*/
        Matrices.projection = glm::ortho(-4.0f*zoom+pan, 4.0f*zoom+pan, -4.0f*zoom, 4.0f*zoom, 0.1f, 500.0f);
        ...
        //Updates the power bar  when ball is fired via keyboard
        if(fired) 
        {
       ...
       while(1)
        ...
        temp= no_of_balls;
        scorex=0.0f;
        //Creates the seven segment display for number of balls
        while(1)
		{
